(function(window, undefined) {
  ...
  window._FS = (function() {
    ...
    fs.prototype._initRum = function() {
      ...
      this.log({ event: "ttfb",        timestamp: t.responseStart });
      this.log({ event: "startRender", timestamp: t.domLoading });
      
      window.addEventListener("DOMContentLoaded", this._logDomReadyAndFlush.bind(this));
      window.addEventListener("DOMContentLoaded", this._logOnLoadFlush.bind(this));
    ...
    fs.prototype._logDomReadyAndFlush = function() {
      this.log({ event: 'domReady', timestamp: window.performance.timing.domComplete });
    };

    // PRIVATE
    fs.prototype._logOnLoadFlush = function() {
      this.log({ event: 'onload', timestamp: window.performance.timing.loadEventEnd });
    };
